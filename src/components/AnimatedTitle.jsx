...
const AnimatedTitle = ({ text }) => {
    ...
    return (
        <div className="relative flex flex-col items-start w-fit max-w-full py-6 overflow-visible">
            ...
            <h1 className="text-[clamp(2rem,7vw,5rem)] font-black flex flex-wrap items-center leading-[1.1] uppercase select-none tracking-tighter">
                {words.map((word, wordIndex) => (
                    <span 
                        key={wordIndex} 
                        className={`inline-block whitespace-nowrap mr-[0.3em] last:mr-0 ${wordIndex === 0 ? 'text-white' : 'text-primary'}`}
                    >
